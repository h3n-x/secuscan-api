...
async def run_header_scan(session: AsyncSession, user_id: str, target: str) -> HeaderScanResult:
    ...
    result = await scan_headers(target)
    await record_scan(session, user_id, target, kind="headers", module_results={"headers": result})
    return result
